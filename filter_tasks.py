...
closed_last_month = Issue.objects.filter(
    status__name="Done", updated_at__gte=timezone.now() - timedelta(days=30)
)
...
# bonus_1
issues_by_fields = Issue.objects.values("id", "summary", "type__name", "status__name")
for issue in Issue.objects.all().order_by("id"):
    types = ", ".join(issue.type.values_list("name", flat=True))
    print(
        f"ID: {issue.pk} | {issue.summary} | Тип: {types} | Статус: {issue.status.name}"
    )


# bonus_2
similar_summary_description = Issue.objects.filter(description=F("summary"))
print(similar_summary_description)


# bonus_3
count_issues_by_type = (
    Issue.objects.values("type__name").annotate(count=Count("id")).order_by("-count")
)
